...
**LCP/FCP가 `null`로 나올 수 있습니다.** headless Chromium이 페인트 엔트리를
간헐적으로 내보내지 않으며, 넓은 뷰포트에서 더 자주 그렇습니다. 페이지는
정상 렌더링되므로 "빠르다"가 아니라 **"측정 안 됨"**으로 읽어야 합니다. 진단기가
이 경우를 그렇게 보고합니다. CLS와 INP는 영향받지 않습니다.
...
```bash
npm run known    # 현재 에러를 "이미 알고 있는 문제"로 기록
```

기존 사이트에 품질 게이트를 켜면 첫날부터 빨간불이고 계속 빨간불입니다. 그런
빌드는 사람이 곧 그냥 넘기게 되고, 그러면 게이트가 장식이 됩니다.

`debug-known-issues.json`에 도입 시점의 에러를 기록해 두고, CI는 **그 이후에
새로 생긴 것만** 실패로 처리합니다. 접근성 위반은 전부 해소되어, 현재 남은 건
web vitals 6건(주로 태그 링크가 400개 넘는 `/tags/`의 FCP)뿐입니다.

시그니처는 측정값을 뭉갭니다 — LCP가 12.6초든 13.9초든 같은 문제입니다. 다만
**개수는 뭉개지 않습니다**: axe 메시지가 "2건 위반"이라 숫자만 뭉개면 2건이
20건이 되어도 계속 통과해 버립니다. 그래서 위반 **규칙 ID**를 시그니처에 함께
넣습니다.

매칭은 동일이 아니라 **부분집합**입니다. 두 규칙 중 하나를 고치면 남은 규칙
집합이 기록된 것의 부분집합이 되므로 통과합니다. 규칙이 **추가**되면 부분집합이
아니므로 잡힙니다. 이 구분이 없으면 고쳤는데 빌드가 깨지고, 그러면 아무도 안
고치게 됩니다.

고친 뒤 `npm run known`으로 다시 기록하면 목록에서 지워집니다.

### CI

`.github/workflows/quality.yml`이 push와 PR마다 정적 점검 → 빌드 → selftest →
브라우저 진단을 실행하고 `debug-report/`를 아티팩트로 올립니다. 로컬에서
실행하는 걸 기억하지 않아도 되도록 하는 게 목적입니다.

죽어 있던 `.travis.yml`은 제거했습니다 — Ruby 2.4, 존재하지 않는 algolia 설정,
없는 `master` 브랜치 기준이었습니다.

전체 실행은 5분 남짓 걸립니다(스로틀 + 21개 조합). 로컬에서 빠르게 확인하려면
`npm run debug:fast`.
