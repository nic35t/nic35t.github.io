...
| `critical-css` | 인라인 CSS가 스타일시트보다 오래됨 (정적 점검) |

### Critical CSS

```bash
npm run critical     # 서버가 떠 있어야 함. scripts/debug.sh가 띄워줍니다
```

첫 화면에 필요한 CSS만 뽑아 `_includes/head/critical-css.html`로 저장하고,
`head.html`이 이걸 인라인합니다. 나머지 `main.css`는 `rel="preload"`로 비차단
로드합니다. 이걸 하기 전엔 브라우저가 **아무것도** 그리지 못한 채 71KB
스타일시트의 왕복을 기다렸고, 그게 FCP의 대부분이었습니다.

추출은 추측이 아니라 브라우저에 물어봅니다. `main.css`의 모든 규칙을 실제 DOM에
대고 테스트해서, 첫 화면(높이의 1.5배까지)에 걸리는 요소가 하나라도 있으면
남깁니다. 사이트의 모든 템플릿 × 모바일·데스크톱 두 폭에서 돌린 뒤 합집합을
취합니다 — 홈 페이지 하나로 뽑으면 글 페이지가 무너집니다.

레이아웃 컨테이너(`#main`, `.sidebar`, `.page__related` 등)는 위치와 무관하게
**항상 포함**합니다. 이게 빠지면 단순히 안 꾸며진 게 아니라 **내용이 엉뚱한
칸에 들어갑니다** — 실제로 글 페이지의 관련글 블록이 작성자 사이드바로 흘러
들어갔고, 그 규칙들은 완성된 페이지에서 한참 아래에 있어 위치 기반 선택으로는
닿지 않았습니다.

**스타일을 고치면 반드시 다시 뽑아야 합니다.** 안 뽑아도 완성된 페이지는
멀쩡하고, 잘못되는 건 main.css가 도착하기 전 찰나뿐이라 **시각 회귀로는 절대
잡히지 않습니다.** 그래서 생성기가 소스 해시를 파일에 적어 두고, `doctor`가
그걸 대조해 오래된 상태면 에러를 냅니다.
